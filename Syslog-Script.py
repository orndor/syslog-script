...
# and creates a .csv file which contains device hostname; loopback, fa0/0, or management vlan (222 or other), logging host, and loggin level
    ...
    with open (file) as in_file: # open the file
        with open('..\Output\output.csv',mode='a') as f: # open the output file again
            for line in in_file: # for each line in the file, check the following:
                ...
                if 'logging host' in line and '!' not in line and 'Add' not in line and 'trusted' not in line: # look for logging host but ignore ignore comment lines
                ...
                if 'logging trap' in line: # look for logging level
                    log_trap = line[13::]
                while interface == '': # interface will be blank on the first run through.  if it matches on any line, it would ignore this while loop the next time through
                    if 'interface Loopback0' in line and '-' not in line: # look for loopback interfaces in routers
                        line = next(in_file)
                        interface = line[-32:27]
                        break # break out of where loop if theres a match
                    if 'interface FastEthernet0/0' in line: # look for fa0/0 interfaces in other devices, jump down two lines and grab the IP
                        ...
                        break # break out of where loop if theres a match
                    if 'interface Vlan222' in line: # look for fa0/0 interfaces in other switches, jump down two lines and grab the IP
                        line = next(in_file)
                        line = next(in_file)
                        interface = line[-32:27].rstrip('\r\n')
                        break # break out of where loop if theres a match
                    if '! ip address ' in line: # look management interfaces in firewalls, jump down a line and grab the IP
                        line = next(in_file)
